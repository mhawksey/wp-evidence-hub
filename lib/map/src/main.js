...
var loadData = function() {
    ...
    queue()
        .defer(d3.json,'wp-content/plugins/wp-evidence-hub/lib/map/data/world-110m.json')
        .defer(dsv, 'wp-content/plugins/wp-evidence-hub/lib/map/data/world-country-names.csv')
        .await(prepareData)
}
...
var selectCountry = function(id, showTooltip){
    ...
    if(showTooltip){
        var tooltip = d3.select('#evidence-map').append('div')
            ...
            .attr('style', 'left:'+ (d3.event.pageX - 50) +'px;top:'+ (d3.event.pageY - 50) +'px')
            .transition()
            .duration(250)
            .style('opacity', 1)
        /* ugly hack */    
...
function displaySankey(slug){
	d3.json(MyAjax.ajaxurl, function(graph) { 
		...
		var link = san.append("g").selectAll(".link")
		  .data(graph.links)
		.enter().append("path")
		  .attr("class", "link")
		  .attr("d", spath)
...
var addTitle = function(){
    svg
        .append('g')
        .attr('id', 'maptitle')
        .attr('transform', 'translate(' + [24, 34] + ')')
		.append("text")
		.attr("text-anchor", "start")  
		.style("font-size", "20px") 
		.text("OER Research Hub - Evidence Map");
}
...
function updateWindow(){
	...
	if (x < 650) {
		 map.attr('transform', 'translate( -300,0)');
		 console.log(map);
	} else {
...
var displayMapData = function(data) {
    ...
    // DATA JOIN
    var positive = map.select('#positiveGroup').selectAll('.positive').data(function(d){
        return data.filter(function(d){ return !isNaN(d.positive)});
    }, function(d){ 
        return d.id;
    });

    var negative = map.select('#negativeGroup').selectAll('.negative').data(function(d){
    ...
    positive
        ...
        .delay(TRANSDURATION)
        .attr('r', function(d){ return positiveMapScale(d.positive);});
    negative
        .transition()
        .duration(TRANSDURATION)
        // includes a short delay because barchart ordering is done first
        ...
        .attr('r', function(d){ return positiveMapScale(d.negative);});
...
var displayTimeseries = function(data, country){
    ...
    var x = d3.time.scale()
        .range([0, width]);
    // color scale maps negative and positive to their respective colors
